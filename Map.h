...
private:
  ...
  class PairComp {
    public:
      bool operator() (const Pair_type &lhs_pair, const Pair_type &rhs_pair){
        Key_compare comp;
        return comp(lhs_pair.first, rhs_pair.first);
      } 
  };
...
public:
  ...
  //       also need to define an appropriate comparator type (PairComp) for th
  ...
  // copy, assign, and destroy Maps.
    ...
    Map(){
      pair_val = new BinarySearchTree<Pair_type, PairComp>;
    }
    ~Map(){
      delete pair_val;
    }
  ...
  //           that element. This ensures the proper value-initialization is 
  //
  ...
  std::pair<Iterator, bool> insert(const Pair_type &val)
  {
    ...
    if(pair_val -> find(val) == pair_val->end())
    {
      pair_val->insert(val);
    }
    return std::make_pair(pair_val->find(val), false);
...
private:
  BinarySearchTree<Pair_type,PairComp> *pair_val;
};
