  ...
  static bool empty_impl(const Node *node) {
   ...
   static Node * max_element_impl(Node *node) {
    ...
    static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
      try {
        min_helper_greater_than(node, val, less);
      } catch (Node * result) {
        return result;
      } 
        return nullptr;
    ...
    static void min_helper_greater_than(Node *node, const T &val, Compare less) {
      if (node == nullptr) {
        return;
      }
      min_helper_greater_than(node->left, val, less);
        if (less(val, node->datum)) {
          throw node;
        }
      min_helper_greater_than(node->right, val, less);
        return;
    }
